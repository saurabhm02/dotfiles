return {
  ...
  config = function()
    ...
    vim.api.nvim_set_keymap("n", "<A-h>", ":lua _TOGGLE_HORIZONTAL()<CR>", opts)

    -- Set key mappings for toggling Node and lazygit terminals
    vim.api.nvim_set_keymap("n", "<A-n>", ":lua _NODE_TOGGLE()<CR>", opts)
    ...
    vim.api.nvim_set_keymap("i", "<A-f>", "<Esc>:lua _TOGGLE_FLOAT()<CR>", opts)
  end,
}
